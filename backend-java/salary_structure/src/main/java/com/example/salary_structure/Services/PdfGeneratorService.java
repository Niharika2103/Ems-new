...
public class PdfGeneratorService {
    ...
    private static final Logger log =
            LoggerFactory.getLogger(PdfGeneratorService.class);
    ...
    public ByteArrayInputStream generatePayslip(SalaryStructure s) {
        ...
        log.info("Generating payslip PDF for employeeId={}", s.getEmployeeId());
        ...
        try {
            ...
            if (pan == null || pan.isEmpty()) {
                pan = "DEFAULT1234";
                log.warn("PAN number missing for employeeId={}, using default password",
                        s.getEmployeeId());
            }
            ...
            BigDecimal daysWorked =
                    s.getDaysWorked() == null ? BigDecimal.ZERO : s.getDaysWorked();
            BigDecimal divisor = new BigDecimal("30");
            ...
            Paragraph pdTitle = new Paragraph(
                    "PER-DAY & EARNED VALUES",
                    new Font(Font.HELVETICA, 14, Font.BOLD)
            );
            document.add(pdTitle);
            ...
            // ================= EARNINGS =================
            Paragraph earnTitle = new Paragraph(
                    "EARNINGS",
                    new Font(Font.HELVETICA, 14, Font.BOLD)
            );
            document.add(earnTitle);
            ...
            // ================= DEDUCTIONS =================
            Paragraph dedTitle = new Paragraph(
                    "DEDUCTIONS",
                    new Font(Font.HELVETICA, 14, Font.BOLD)
            );
            document.add(dedTitle);
            ...
            log.info("Payslip PDF generated successfully for employeeId={}",
                    s.getEmployeeId());

        } catch (Exception e) {
            log.error("Error while generating payslip PDF for employeeId={}",
                    s.getEmployeeId(), e);
        }
    ...
    private PdfPCell cell(Object text, boolean bold) {
        ...
        PdfPCell cell = new PdfPCell(
                new Phrase(String.valueOf(text), font)
        );
        cell.setPadding(6);
        return cell;
    }
