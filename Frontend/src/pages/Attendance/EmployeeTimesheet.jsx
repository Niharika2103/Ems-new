...
export default function EmpTimesheet() {
  ...
  const [shift1, setShift1] = useState(Array(7).fill(0));
  const [shift2, setShift2] = useState(Array(7).fill(0));
  const [shift3, setShift3] = useState(Array(7).fill(0));
...
useEffect(() => {
  ...
  const formattedDate = new Date(weekStart)
    .toISOString()
    .split("T")[0];

  getShiftAssignmentbyemployeeApi(employeeId, formattedDate)
    .then((res) => {
