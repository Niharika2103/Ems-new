...
const ProjectTable = () => {
  ...
  const handleFinishProject = async (record) => {
    ...
    try {
      await dispatch(ProjectFinish(projectId)).unwrap();
      ...
      toast.error(
        typeof err === "string"
          ? err
          : err?.message || "Failed to finish project"
      );
    }
  };
      ...
      render: (_, record) => {
        ...
        return (
          <Button
            ...
            icon={isCompleted ? <CheckCircleOutlined /> : <SyncOutlined spin />}
            disabled={isCompleted}
            onClick={() => handleFinishProject(record)}
          >
            {isCompleted ? "Completed" : "In Progress"}
          </Button>
        );
